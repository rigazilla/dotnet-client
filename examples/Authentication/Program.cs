...
namespace Authentication
{
    ...
    class Program
    {
        const string HOTROD_HOST = "127.0.0.1";
        const int HOTROD_PORT = 11222;
        const string AUTH_CACHE = "authCache";
        public const string REALM = "ApplicationRealm";
        const string mech = "EXTERNAL";

        static void Main(string[] args)
        {
            ConfigurationBuilder conf = new ConfigurationBuilder();
            IRemoteCache<string, string> roCache = InitCache("reader", "password", mech, AUTH_CACHE);
            IRemoteCache<string, string> rwCache = InitCache("writer", "somePassword", mech, AUTH_CACHE);
            roCache.Put("K1", "V1");
            roCache.Get("K1");
            roCache.Clear();
        ...
        protected static IRemoteCache<String, String> InitCache(string user, string password, string authMech, string cacheName = AUTH_CACHE)
        {
            ConfigurationBuilder conf = new ConfigurationBuilder();
            conf.AddServer()
                    .Host(HOTROD_HOST)
            ...
            conf.Security().Authentication()
                                .Enable()
                                .SaslMechanism(authMech)
                                .ServerFQDN("node0")
                                .SetupCallback(user, password, REALM);
            conf.Ssl().Enable().ServerCAFile("infinispan-ca.pem").ClientCertificateFile("truststore_client.p12");
